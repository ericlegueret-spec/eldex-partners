...
<div style="max-width:600px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;border:1px solid #E0D8CC;">
  ...
  <div style="padding:28px;">
    ...
    <div style="text-align:center;margin-top:24px;">
      <a href="mailto:${email||''}" style="display:inline-block;background:#1B2A4A;color:#fff;font-weight:700;font-size:14px;padding:14px 28px;border-radius:8px;text-decoration:none;">Répondre à ${nom||'ce contact'} →</a>
    </div>
  </div>
</div>
</body></html>`;
...
<div style="max-width:600px;margin:0 auto;padding:32px 16px;">
  ...
  <div style="background:#1B2A4A;border-radius:0 0 12px 12px;padding:16px 32px;">
    ...
    <p style="color:#8899BB;font-size:11px;margin:4px 0 0;">ericlegueret.com</p>
  </div>
</div>
</body></html>`;
  ...
  try {
    const result = await transporter.sendMail({
      from: '"ELDEX Partners" <ericlegueret@gmail.com>',
      to: GMAIL_USER,
      subject,
      html: notifHtml
    });
    ...
    if (email && email.includes('@') && email !== GMAIL_USER) {
      await transporter.sendMail({
        from: '"Eric Leguéret" <ericlegueret@gmail.com>',
        to: email,
        subject: isContact
